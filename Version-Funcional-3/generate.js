...
const COMPANY = {
  ...
  // ── Colores de marca ─────────────────────────────────────────
  // Cambia SOLO estos dos y el header/footer se actualizan solos.
  // Recuerda mantenerlos en sync con --navy-base y --resalt-base
  // en styles-corporate.css
  COLOR_NAVY: '#0b2929',
  COLOR_RESALT: '#3a77c8',
...
const DATA = {
  ...
  invoice: {
    ...
    SENDER_NAME: "Daniel Steven Campaz Ocoro",
...
async function generatePDF({ type, logoPath, outputPath }) {
  ...
  // ── Header y Footer inyectados por Puppeteer en CADA página ──
  ...
  const footerHtml = `
    ...
    ">
      <span style="color:rgba(255,255,255,.70)">${companyName} · ${companyWeb}</span>
      <span style="
      ...
      <span style="color:rgba(255,255,255,.70)">${docNumber} · ${docDate}</span>
    </div>`;
